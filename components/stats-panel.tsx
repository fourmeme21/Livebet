...
export function StatsPanel() {
  // FIX: getSettledBets() her çağrıda yeni array üretir → sonsuz döngü
  // placedBets'i seç, filtreyi burada uygula
  const placedBets = useBettingStore((s) => s.placedBets)
  const settledBets = placedBets.filter((b) => b.status !== 'pending')

  const totalWon = settledBets
    .filter((b) => b.status === 'won')
    .reduce((sum, b) => sum + b.potential, 0)
    ...
    <div className="space-y-4 rounded-lg bg-card p-4 shadow-sm">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          ...
          <p
            className={`text-lg font-mono font-bold ${
              profit >= 0 ? 'text-odds-positive' : 'text-odds-negative'
            }`}
          >
        ...
        <div>
          ...
          <p className="text-lg font-mono font-bold text-odds-positive">
            {formatCurrency(totalWon)}
          </p>
        </div>
        <div>
          <p className="text-xs text-muted-foreground">Toplam Kayıp</p>
          <p className="text-lg font-mono font-bold text-odds-negative">
